...
#     def compute_reward_meta(self, d_prev, d_now, dz_acc,
#                             ig_chosen, collided, frontier_score):
#         r  = 4.0*(d_prev - d_now)         # progreso al goal
#         r += 2.0*ig_chosen                # IG del frontier elegido
#         r += 1.5*frontier_score           # frontier “bien posicionado”
#         r -= 0.1*dz_acc                   # coste energético
#         if collided: r -= 30.0
#         return np.clip(r/SCALE_META, -CLIP_REW, CLIP_REW)
...
#     #     h_arr, hm_info = gridmap_to_numpy(self.height_map_msg)
#     #     rrt_area = (cp[0]-2, cp[1]-2, cp[0]+2, cp[1]+2)  # 4×4 m detrás
#     #     best, best_cost = None, np.inf
#     #     for _ in range(200):
#     #         angle = random.uniform(-math.pi+0.3, -0.3)   # semicírculo trasero
#     #         r = random.uniform(0.8, 2.0)
#     #         cand = (cp[0]+r*math.cos(yaw+angle),
#     #                 cp[1]+r*math.sin(yaw+angle))
#     #         if not clearance_ok(self.grid_dyn, self.grid_msg.info, cand, CLEAR_MIN):
#     #             continue
#     #         if not bres_line_free(self.grid_dyn, self.grid_msg.info, cp, cand):
#     #             continue
#     #         _, slope, _ = slope_ok(h_arr, hm_info, cp, cand)
#     #         cost = r + 5*slope
#     #         if cost < best_cost:
...
#             # waypoint = (cp[0] + mu[0]*R_META, cp[1] + mu[1]*R_META)
#             std_m  = tf.exp(self.meta_log_std)
#             logp_m = -0.5 * np.sum(((mu-act)/std_m.numpy())**2 +
#                                 2*np.log(std_m) + np.log(2*np.pi))
#             val_m  = float(self.meta_value_net([big_patch[None,...],
#                                                 vec_goal[None,...]])[0,0])

#             self.meta_obs_buf.append((big_patch, vec_goal))
#             self.meta_act_buf.append(mu)
#             self.meta_logp_buf.append(logp_m)
#             self.meta_val_buf.append(val_m)
...
#                     frontier_score = self.meta_frontier_score,
...
STD_DECAY         = 0.995
MAX_EPISODES      = 400
MAX_STEPS_EP      = 1000                        # Máximo de pasos por episodio (bajo nivel)
MAX_TILT          = 1.0
WHEEL_BASE        = 1.35
DELTA_MAX         = math.radians(32)
KP_SPEED          = 10.0
...
# ---------------------------------------------------------------
    ...
    def teacher_waypoint(self, cp):
        # 1) Goal directo si línea libre
        if bres_line_free(self.grid_dyn, self.grid_msg.info, cp, self.goal):
            return self.goal, "GOAL"

        # 2) Mejor frontier por score IG – d_robot – d_goal
        f, _ = self.select_frontier(cp, self.grid_dyn, self.grid_msg.info)
        if f is not None:
            return f, "FRONTIER"
...
# ---------- Pure-Pursuit + lógica jerárquica (10 Hz) --------------------
    def step(self):
        ...
        # si llegamos al final del path local, solicita nuevo wp
    ...
    def update_ppo(self):
        ...
        with self.writer.as_default():
            ...
            tf.summary.scalar("low/loss_critic", float(v_loss),    step=self.episode)
            tf.summary.scalar("low/policy_std", float(new_std[0]), step=self.episode)
        self.policy.save_weights(RUN_DIR / f"policy_low_ep.weights.h5")
        self.get_logger().info(f"[LOW PPO] ep={self.episode} σ={float(new_std[0]):.3f}")
        self.episode += 1
        self.total_updates += 1
    ...
    def update_meta_ppo(self):
        ...
        with self.writer.as_default():
            ...
            tf.summary.scalar("meta/loss_critic", float(v_loss_m),    step=self.meta_episode)
            tf.summary.scalar("meta/policy_std", float(new_std_m[0]), step=self.meta_episode)
        self.meta_policy.save_weights(RUN_DIR / f"policy_meta_ep.weights.h5")
        self.get_logger().info(f"[META PPO] ep={self.meta_episode} σ={float(new_std_m[0]):.3f}")
        self.meta_episode += 1
        self.meta_total_upd += 1
