...
class VoxelGridNode(Node):
    def __init__(self):
        ...
        # === Filtro AABB opcional ===
        self.declare_parameter("exclude_box_enabled", True)
        self.declare_parameter("exclude_box_frame", "base_link")
        self.declare_parameter("exclude_box_center_x", 0.35)
        self.declare_parameter("exclude_box_center_y", 0.0)
        self.declare_parameter("exclude_box_center_z", 0.35)
        self.declare_parameter("exclude_box_size_x", 3.1)
        self.declare_parameter("exclude_box_size_y", 1.8)
        self.declare_parameter("exclude_box_size_z", 1.4)
        self.declare_parameter("use_latest_tf", True)
        self.declare_parameter("tf_timeout", 0.07)

        # === Marker (visualización de la caja en RViz) ===
        self.declare_parameter("publish_marker", True)
        self.declare_parameter("marker_topic", "/voxel_exclusion_marker")
        self.declare_parameter("marker_rgba", [0.0, 1.0, 1.0, 0.25])  # cian translúcido
        ...
        rgba_param = self.get_parameter("marker_rgba").get_parameter_value().double_array_value
        rgba = list(rgba_param) if len(rgba_param) == 4 else [0.0, 1.0, 1.0, 0.25]
        self.marker_color = dict(r=rgba[0], g=rgba[1], b=rgba[2], a=rgba[3])
        self.marker_rate = float(self.get_parameter("marker_rate_hz").value)
        ...
        if self.publish_marker:
            period = max(0.05, 1.0 / max(0.1, self.marker_rate))
            self.marker_timer = self.create_timer(period, self.publish_marker_cb)

        self.get_logger().info(
            f"VoxelGrid: {in_topic} -> {out_topic} (leaf={self.leaf} m). "
            f"Caja: {'ON' if self.exclude_enabled else 'OFF'} en '{self.exclude_frame}'. "
            f"Marker: {'ON' if self.publish_marker else 'OFF'} -> {self.marker_topic}"
        )
    ...
    def cb(self, msg: PointCloud2):
            ...
            if self.exclude_frame and self.exclude_frame != src_frame:
                ...
                if R_t is None:
                    if not self.warned_tf:
                        self.get_logger().warn(
                            f"Sin TF {self.exclude_frame}<-{src_frame}. "
                            f"Omito filtro de caja (activa use_latest_tf o revisa TFs)."
                        )
                        self.warned_tf = True
                else:
                    R, t = R_t
        ...
        # Voxelización por cuantización
        q = np.floor(pts / self.leaf).astype(np.int32)
        _, keep_idx = np.unique(q, axis=0, return_index=True)
        out = pts[keep_idx]

        # Empaquetar PointCloud2
        header = Header()
        header.stamp = msg.header.stamp
        header.frame_id = self.frame_id or msg.header.frame_id
    ...
    # ---- Publicación del Marker de la caja ----
    def publish_marker_cb(self):
        if not self.publish_marker:
            return

        m = Marker()
        m.header.frame_id = self.exclude_frame
        m.header.stamp = self.get_clock().now().to_msg()
        m.ns = "voxel_exclusion"
        m.id = 0
        m.type = Marker.CUBE
        m.action = Marker.ADD
        m.pose.position.x = float(self.ex_center[0])
        m.pose.position.y = float(self.ex_center[1])
        m.pose.position.z = float(self.ex_center[2])
        # Caja alineada a ejes => orientación identidad
        m.pose.orientation.x = 0.0
        m.pose.orientation.y = 0.0
        m.pose.orientation.z = 0.0
        m.pose.orientation.w = 1.0
        m.scale.x = float(self.ex_size[0])
        m.scale.y = float(self.ex_size[1])
        m.scale.z = float(self.ex_size[2])
        m.color.r = float(self.marker_color['r'])
        m.color.g = float(self.marker_color['g'])
        m.color.b = float(self.marker_color['b'])
        m.color.a = float(self.marker_color['a'])
        m.lifetime = Duration(seconds=0.0).to_msg()  # infinito
