# #############################################
# # ARGOSJ8_IA – Dockerfile                   #
# # Ubuntu 22.04 + ROS 2 Humble + GUI + LCM   #
# #############################################
...
# # 2) Instalar locales, Qt6 y dependencias básicas
# RUN apt-get update && \
...
# # 3) Añadir repositorio ROS 2 Humble
...
# # 4) Añadir repositorio OSRF (Gazebo 11)
...
# # 5) Instalar ROS 2 Desktop Full + colcon + rosdep + Gazebo + LCM + GeographicLib
# RUN apt-get update && \
#     apt-get install -y --no-install-recommends \
...
#     rm -rf /var/lib/apt/lists/*
# ########################################################################
# # 5.b) Compilar e instalar LCM 1.5 (ahora con CMake)                    #
# ########################################################################
# RUN git clone --depth 1 --branch v1.5.0 https://github.com/lcm-proj/lcm.git /tmp/lcm && \
#     cmake -S /tmp/lcm -B /tmp/lcm/build \
...
# # 6) Stack Python coherente
# RUN python3 -m pip install --no-cache-dir --upgrade \
#       pip setuptools~=69.5 packaging wheel \
#       "numpy<1.25" cython opencv-python-headless

# # 7) Copiar código y compilar tu workspace
# WORKDIR /ros2_ws
# COPY src ./src
